    ...
    if (!doctor.isActive) {
      ...
      try {
        ...
        await session.withTransaction(async () => {
          ...
          const hospital = await Hospital.findById(req.hospital._id);
          if (
            !hospital.specialization.find(
              (spec) => spec == doctor.specialization
            )
          ) {
        ...
        //console.log({doctor});
        res.send(
          `${doctor} is already exists but we added it to your hospital`
        );
      } catch (error) {
        console.log(error);
      ...
      //res.send(doctor);
    } else if (doctor.hospital == req.hospital._id)
      return res
        .status(400)
        .send(`${doctor.name} is already working in your hospital`);
    else {
      return res
        .status(400)
...
module.exports.activateDoctor = async (req, res) => {
  ...
  doctor.save();
  res.send(
    `${doctor.name}'s account is activated and assigned to your hospital`
  );
};
...
module.exports.deactivateDoctor = async (req, res) => {
  ...
  if (!doctor) return res.status(404).send("Doctor not Found!");
  if (doctor.hospitalID != req.hospital._id)
    return res.status(400).send("You are not autheraized to deactivate him");
  doctor.hospitalID = null;
  doctor.isActive = false;
  doctor.save();
  res.send(
    `${doctor.name}'s account is deactivated and removed from your hospital`
  );
};
/*
DONE:
...
module.exports.addReceptionist = async (req, res) => {
  ...
  if (receptionist) {
    if (!receptionist.isActive) {
      ...
      return res
        .status(400)
        .send(`${receptionist.name} is already working in your hospital`);
    } else {
...
module.exports.deactivateReceptionist = async (req, res) => {
  ...
  if (receptionist.hospitalID != req.hospital._id)
